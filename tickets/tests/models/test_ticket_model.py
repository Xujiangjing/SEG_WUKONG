...
class TicketModelTest(TestCase):
    def setUp(self):
        self.departmentS = Department.objects.create(
            name="welfare", description="txt", responsible_roles="specialist"
        )
        self.department = Department.objects.create(
            name="admissions", description="txt", responsible_roles="program_officers"
        )
    ...
    def test_ticket_str(self):
        self.assertEqual(
            str(self.ticket),
            f"Ticket {self.ticket.id}: {self.ticket.title} ({self.ticket.status})",
        )
